# Design: ProjectState Parallel-Agent Worktree Orchestrator
...
ProjectState currently assumes a single agent owns the worktree during a slice. When two or more agents run concurrently, existing scripts panic (exit non-zero / report "not proven") because:

- `projectstate_worktree_guard.py` sees another agent's uncommitted files as ambiguous dirt.
...
A small JSON file `.projectstate/agent.context` lives in the root of each agent worktree:

```json
{
  "schema": "projectstate.agent_context.v1",
  "agent_id": "agent-7f3a",
  "slice_id": "BL-BROWSER-002",
  "reservation_ref": "refs/projectstate/reservations/bl-browser-002-agent-7f3a-abc12",
  "worktree_path": "/home/ff/Documents/Projects/ProjectState_Template/.worktrees/bl-browser-002-agent-7f3a-abc12",
...
When existing ProjectState scripts detect this file, they treat uncommitted changes in the worktree as *intended slice work* rather than ambiguous shared-worktree dirt.

### Reservation ref

Reservations are stored as lightweight git refs under `refs/projectstate/reservations/<branch-name>`. Each ref points to the base commit at reservation time and contains the agent context as ref log message / annotation.
...
## New Component: `scripts/projectstate_agent_worktree.py`
...
3. Run equivalent checks to `projectstate_worktree_guard.py`, but in agent context:
...
1. Load agent.context.
2. Run `projectstate_handoff.py` from inside the worktree.
...
3. Run `projectstate_remote_closure_finalizer.py --pr <N>`.
4. On success, remove reservation ref and worktree.
5. On failure, leave worktree intact for debugging and report path.

### Cleanup flow

1. Enumerate `refs/projectstate/reservations/` and `git worktree list`.
...
### `scripts/projectstate_closure_check.py`
...
- Add a job or step that runs `python3 scripts/projectstate_agent_worktree.py --dry-run start --slice-id CI-SMOKE-001` to prove the orchestrator works in a fresh checkout.
...
- `skills/close-slice/SKILL.md`: update to use `projectstate_agent_worktree.py start/close`.
...
- Agent worktree is opt-in via `projectstate_agent_worktree.py start` but becomes the recommended default in `skills/close-slice/`.
- Downstream repos that adopt the template receive `scripts/projectstate_agent_worktree.py` through `init_template.py` and `projectstate_upgrade.py`.
...
1. Two agents can run `projectstate_agent_worktree.py start` for different slices and not collide.
2. An agent can leave dirty files in its worktree and still pass `projectstate_audit.py --strict`.
3. `projectstate_handoff.py` in an agent worktree reports the correct agent_id, slice_id, and branch.
4. `projectstate_remote_closure_finalizer.py` detects if another agent interleaved a push and reports it clearly.
