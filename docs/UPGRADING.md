...
Current StateSpec spec version: `statedd-template-v5`

Use this guide when bringing an existing StateSpec repo forward without overwriting local project truth.

## Upgrade Rules

- Treat `VERSION` as the canonical StateSpec template version.
- Add `repo_role` and `statedd_mode` when upgrading older StateSpec state files.
- Use `repo_role: template_repository` and `statedd_mode: template-maintenance` only for the StateSpec template repository itself.
...
- Existing project `CHANGELOG.md` files may be product history, not StateSpec template history. Do not overwrite them during adoption without explicit human approval.
