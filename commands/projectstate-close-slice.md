---
command: "projectstate-close-slice"
gate_level: 2
evidence_max: 8
...
# /projectstate-close-slice — Close Implementation Slice

**When to use:** After implementation is complete, before declaring a slice closure-grade.

**Triggers:**
- Human types `/projectstate-close-slice`
...
2. If the local gate passes:
   ...
   - For confirmed `agent_after_green`, run `scripts/projectstate_finish_slice.py` with
...
`projectstate_agent_worktree.py abandon --reason <failed_preflight|superseded|operator_cancelled>`;
