---
command: "projectstate-release-gate"
gate_level: 3
evidence_max: 8
...
# /projectstate-release-gate — Release Gate

**When to use:** Before any deployment, release, or user-facing delivery.

**Triggers:**
- Human types `/projectstate-release-gate`
...
3. Run `python3 scripts/projectstate_runtime_proof.py --evidence-dir docs/evidence/<slice> --url <endpoint>` — capture deployment runtime
4. Run `python3 scripts/projectstate_runtime_truth_check.py --artifact docs/evidence/<slice>/runtime_identity.json --expected-endpoint <endpoint>` — verify matches
5. Run `scripts/projectstate_evidence_type_check.py` — verify release evidence
6. Run `scripts/projectstate_remote_closure_finalizer.py` — verify pushed PR/CI state
