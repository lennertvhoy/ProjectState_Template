...
PROBES = [
    ProbeTask(
        ...
        expected_commands=["projectstate-failure-scan", "projectstate-quality-freeze", "projectstate-close-slice"],
        expected_gates=["projectstate_quality_gate.py", "projectstate_closure_check.py", "projectstate_instruction_lint.py"],
    ...
    ProbeTask(
        ...
        expected_commands=["projectstate-close-slice"],
        expected_gates=["projectstate_quality_gate.py", "projectstate_closure_check.py", "projectstate_runtime_truth_check.py", "projectstate_evidence_type_check.py"],
        setup="echo '# New feature\nprint(\"hello\")' > feature.py",
        verify="python scripts/projectstate_quality_gate.py"
    ),
    ProbeTask(
        ...
        expected_commands=["projectstate-ingest-bad-event", "projectstate-failure-scan"],
        expected_gates=["projectstate_failure_scan_check.py"],
    ...
    ProbeTask(
        ...
        expected_commands=["projectstate-close-slice"],
        expected_gates=["projectstate_validate_schema.py", "projectstate_instruction_lint.py"],
        setup="cp PROJECT_STATE.yaml PROJECT_STATE.yaml.bak",
        verify="python scripts/projectstate_validate_schema.py"
    ),
    ProbeTask(
        type=ProbeType.FAILURE_SCAN,
        description="Run failure scan before risky refactor",
        expected_skills=["failure-scan"],
        expected_commands=["projectstate-failure-scan"],
        expected_gates=[],
    ...
    ProbeTask(
        ...
        expected_commands=[],
        expected_gates=["projectstate_runtime_truth_check.py"],
        setup=(
            "python scripts/projectstate_runtime_proof.py --no-runtime-required "
            "--evidence-dir docs/evidence/runtime-proof-probe"
        ),
        verify=(
            "python scripts/projectstate_runtime_truth_check.py "
...
class ProbeGuidance:
    def __init__(self, root: Path, verbose: bool = False):
        ...
        self._temp_dir = tempfile.TemporaryDirectory(prefix="projectstate_probe_")
    ...
    def run_all(self) -> Tuple[int, List[Dict]]:
        """Run all probes."""
        print("=" * 50)
        print("ProjectState Probe Guidance")
        print("=" * 50)
...
def main():
    parser = argparse.ArgumentParser(description="ProjectState Probe Guidance")
