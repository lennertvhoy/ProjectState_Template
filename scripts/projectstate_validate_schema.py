#!/usr/bin/env python3
"""Validate ProjectState state, evidence, runtime, and handoff contracts.

The validator intentionally stays stdlib-only. It implements the small JSON
Schema subset used by this template and a tiny YAML parser for the ProjectState YAML
...
class ProjectStateYamlError(ValueError):
    pass
...
def preprocess_yaml(text: str) -> list[tuple[int, str, int]]:
    ...
    for lineno, raw in enumerate(text.splitlines(), start=1):
        ...
        if "\t" in raw[:indent]:
            raise ProjectStateYamlError(f"line {lineno}: tabs are not supported in indentation")
...
def parse_yaml_text(text: str) -> Any:
    ...
    if index != len(lines):
        ...
        raise ProjectStateYamlError(f"line {lineno}: unexpected content after YAML block: {content}")
    return value
...
def split_key_value(content: str, lineno: int) -> tuple[str, str]:
    if ":" not in content:
        raise ProjectStateYamlError(f"line {lineno}: expected key: value")
    key, value = content.split(":", 1)
    key = key.strip()
    if not key:
        raise ProjectStateYamlError(f"line {lineno}: empty mapping key")
...
def parse_mapping(lines: list[tuple[int, str, int]], index: int, indent: int) -> tuple[dict[str, Any], int]:
    ...
    while index < len(lines):
        ...
        if key in mapping:
            raise ProjectStateYamlError(f"line {lineno}: duplicate mapping key {key!r}")
        index += 1
        if value_text in {"|", ">"}:
...
def validate_json_schema(
    ...
    *,
    ...
    semantics = schema.get("projectstate_semantics")
    if isinstance(semantics, dict):
        issues.extend(validate_projectstate_semantics(value, semantics, path))

    return issues


def validate_projectstate_semantics(value: Any, semantics: dict[str, Any], path: str) -> list[ValidationIssue]:
    issues: list[ValidationIssue] = []
    ...
    if semantics.get("repo_role_mode_compatibility") is True and isinstance(value, dict):
        ...
        if isinstance(workflow, dict):
            ...
            mode = workflow.get("projectstate_mode") or workflow.get("repo_mode")
            if role == "template_repository" and mode != "template-maintenance":
                issues.append(
                    ValidationIssue(
                        f"{path}.workflow.projectstate_mode",
                        "template_repository must use projectstate_mode: template-maintenance",
                    )
                )
            if role == "downstream_project" and mode == "template-maintenance":
                issues.append(
                    ValidationIssue(
                        f"{path}.workflow.projectstate_mode",
                        "downstream_project cannot use projectstate_mode: template-maintenance",
                    )
                )
    if semantics.get("projectstate_mode_matches_repo_mode") is True and isinstance(value, dict):
        workflow = value.get("workflow")
        if isinstance(workflow, dict) and workflow.get("projectstate_mode") != workflow.get("repo_mode"):
            issues.append(
                ValidationIssue(
                    f"{path}.workflow.repo_mode",
                    "repo_mode must match projectstate_mode",
                )
            )
...
def root_targets(root: Path) -> list[tuple[Path, Path, bool]]:
    ...
    # Assets manifest may exist under either canonical or legacy filename.
    assets_manifest = resolve_assets_manifest(root)
    if assets_manifest is not None:
        targets.append((assets_manifest, schema_path(root, "projectstate_assets.schema.json"), False))
    targets.extend([
                ...
                if candidate.exists():
                    if name == "runtime_identity.json":
                        ...
                        if (
                            ...
                            and runtime_payload.get("schema")
                            in (
                                "projectstate.runtime_identity.v2",
                                "statedd.runtime_identity.v2",  # backward-compat alias
                            )
                        ):
...
def validate_manifest_catalog_consistency(root: Path) -> list[ValidationIssue]:
    ...
    if manifest_path is None or not catalog_path.exists():
        return []
    manifest = load_json_file(manifest_path)
    if not isinstance(manifest, dict) or manifest.get("schema") not in (
        "projectstate.runtime_assets.v2",
        "statedd.runtime_assets.v2",  # backward-compat alias
    ):
        return []
...
def validate_root(root: Path, quiet: bool) -> int:
    ...
    if not quiet:
        print("============================================================")
        print("PROJECTSTATE SCHEMA VALIDATION")
        print("============================================================")
    try:
    ...
    for path, schema_path, required in targets:
        ...
        except (OSError, UnicodeDecodeError, json.JSONDecodeError, ProjectStateYamlError, ContractError) as exc:
    ...
    if all_issues:
        ...
        return 1
    if not quiet:
        print("PASSED: All ProjectState schema checks passed")
    return 0
