#!/usr/bin/env python3
"""Pre-slice and closure guard for ProjectState git worktree state.
...
def print_report(
    ...
    print("ProjectState Worktree Guard")
    print(f"Mode: {mode}")
    print()
    print("Repo truth")
    if agent_context:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ProjectState worktree isolation guard")
    ...
    parser.add_argument(
        "--agent-context",
        help="Path to agent.context JSON file (default: .projectstate/agent.context in repo root)",
    )
    parser.add_argument(
        "--warn-only",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if code != 0:
        print(f"ProjectState Worktree Guard\nMode: {args.mode}\n\nBlocking problems\n- {error}")
        return code
