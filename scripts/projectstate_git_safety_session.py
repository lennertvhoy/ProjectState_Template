#!/usr/bin/env python3
"""Session-state enforcement for the ProjectState Git safety preflight.

The executable preflight owns the transition between an external read-only
latch and a short-lived mutation permit. ProjectState writers consume that permit;
...
LATCH_SCHEMA = "projectstate.git_safety_latch.v1"
PERMIT_SCHEMA = "projectstate.git_safety_permit.v1"
...
class MutationBlocked(RuntimeError):
    """A ProjectState-managed write lacks a valid session mutation permit."""
...
def default_state_root() -> Path:
    configured = os.environ.get("PROJECTSTATE_GIT_SAFETY_STATE_ROOT", "").strip()
    if not configured:
    ...
    uid = effective_uid()
    return Path(tempfile.gettempdir()) / f"projectstate-git-safety-{uid if uid is not None else 'unknown'}"
...
def state_lock(root: Path | None = None) -> Iterator[Path]:
    if fcntl is None:
        raise MutationBlocked("POSIX file locking is unavailable; writable ProjectState sessions are blocked")
...
def latch_payload(
    *,
    ...
    return {
        ...
        "decision_schema": "projectstate.git_safety_decision.v1",
...
def require_mutation_permit(
    ...
    *,
    ...
    """Require a valid external permit before a ProjectState-managed repository write."""
