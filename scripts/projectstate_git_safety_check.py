...
SCHEMA_ID = "projectstate.git_safety_report.v1"
...
def probe_directory(path: Path) -> tuple[dict[str, Any], str | None]:
    ...
    probe = path / f".projectstate-write-probe-{token}"
    ...
    try:
        ...
        created = True
        try:
            os.write(descriptor, b"projectstate git safety write probe\n")
            os.fsync(descriptor)
        finally:
            os.close(descriptor)
    except OSError as exc:
...
def build_isolation(
    ...
    rules = [
        ...
        "read_only never permits ProjectState-managed mutation",
    ]
    return {
        "requested_mode": mode,
        "selected_mode": mode,
...
def validate_report(report: dict[str, Any]) -> list[str]:
    try:
        from projectstate_validate_schema import validate_json_schema
    except ImportError:
    ...
    *,
    ...
    report: dict[str, Any] = {
        ...
        "decision": {
            ...
            "enforcement_scope": "projectstate_managed_session_permit",
            "operation_class": args.operation_class,
...
def print_human(report: dict[str, Any]) -> None:
    print("ProjectState Git Safety Preflight")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ProjectState fail-closed Git safety preflight")
    ...
    try:
        with state_lock(Path(args.latch_root).resolve()) as locked_state_root:
            try:
                ...
                diagnostic = f"ProjectState Git safety inspection failed: {exc}"
                ...
                diagnostic = f"ProjectState Git safety preflight crashed: {exc}"
                latch_error = persist_unreported_failure(
                    locked_state_root, requested, resolved, diagnostic
                )
                print(diagnostic, file=sys.stderr)
            ...
            if schema_issues:
                ...
                print("ProjectState Git safety report failed its schema:", file=sys.stderr)
            ...
            if schema_issues:
                ...
                print("ProjectState Git safety final report failed its schema:", file=sys.stderr)
                for item in schema_issues:
                    print(f"- {item}", file=sys.stderr)
                if latch_error:
                    print(f"Read-only latch persistence also failed: {latch_error}", file=sys.stderr)
        ...
        print(f"ProjectState Git safety session-state lock failed closed: {exc}", file=sys.stderr)
        return 2
