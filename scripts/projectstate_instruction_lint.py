#!/usr/bin/env python3
"""
ProjectState Instruction Linter
...
class InstructionLinter:
    ...
    def _check_missing_failure_cases(self, analysis: FileAnalysis):
        ...
        if not (analysis.path.startswith("skills/") or analysis.path.startswith("commands/")):
            return
        # Backward-compat command alias stubs (post StateDD -> ProjectState rename)
        # advertise themselves via an `alias_for:` frontmatter field. They are
        # routing files, not workflow content, and intentionally omit failure
        # cases because the canonical command already covers them.
        if "alias_for:" in "\n".join(analysis.lines[:10]):
            return
        content = "\n".join(analysis.lines).lower()
...
def main():
    parser = argparse.ArgumentParser(description="ProjectState Instruction Linter")
