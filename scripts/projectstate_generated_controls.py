...
update live state, and end with a precise handoff. Before repository or ProjectState
mutation, run `scripts/projectstate_git_safety_check.py` and use a full clone for
containers or independent agents. The integration agent owns the final slice
branch; subagents return bounded commits and do not edit global ProjectState truth.
...
`scripts/projectstate_finish_slice.py`. Never infer a CI-unavailable override or
silently change the confirmed mode.
"""


def render_downstream_workflow(required_gate_level: int) -> str:
    return f"""name: ProjectState
  ...
  branch-head:
    ...
    steps:
      ...
      - uses: actions/setup-python@a26af69be951a213d495a4c3e4e4022e16d87065
        with:
          python-version: "3.13"
      - run: python3 scripts/projectstate_quality_gate.py --gate-level {required_gate_level} --conformance
