#!/usr/bin/env python3
"""ProjectState strong-isolation orchestrator for independent coding agents.
...
from projectstate_git_safety_session import (
    MutationBlocked,
...
try:
    from projectstate_workspace_inventory import (
        build_inventory,
        managed_active_root,
        managed_clone_path,
    ...
    from scripts.projectstate_workspace_inventory import (
        build_inventory,
        managed_active_root,
        managed_clone_path,
        managed_quarantine_root,
...
GIT_SAFETY_SCRIPT = ROOT / "scripts" / "projectstate_git_safety_check.py"
AGENT_CONTEXT_SCHEMA = "projectstate.agent_context.v2"
RESERVATION_REF_PREFIX = "refs/projectstate/reservations/"
...
def generate_agent_id() -> tuple[str, str]:
    env_id = os.environ.get("PROJECTSTATE_AGENT_ID", "").strip()
    if not env_id:
...
def reject_nested_or_unmanaged_source(source: Path) -> None:
    ...
    context_path = source / ".projectstate" / "agent.context"
    ...
    if path_is_within(source, workspace_state_root()):
        raise RuntimeError(
            "Nested agent isolation is forbidden from inside the managed ProjectState workspace root."
        )
...
def load_agent_context(path: Path) -> tuple[int, dict[str, Any], str]:
    context_path = path / ".projectstate" / "agent.context" if path.is_dir() else path
    try:
...
def write_agent_context(target: Path, context: dict[str, Any]) -> None:
    projectstate_dir = target / ".projectstate"
    projectstate_dir.mkdir(parents=True, exist_ok=True)
    context_path = projectstate_dir / "agent.context"
    descriptor, temporary_name = tempfile.mkstemp(prefix="agent.context.", dir=projectstate_dir)
    os.close(descriptor)
...
def cmd_guard(args: argparse.Namespace) -> int:
    ...
    if args.mode == "closure" and changed:
        ...
        return 1
    print("ProjectState Agent Isolation Guard")
...
def cmd_handoff(args: argparse.Namespace) -> int:
    ...
    handoff_script = ROOT / "scripts" / "projectstate_handoff.py"
...
def release_receipt(
    *,
    ...
    return {
        "schema": "projectstate.isolation_release.v1",
...
def emit_release_receipt(receipt: dict[str, Any], output_format: str) -> None:
    if output_format == "json":
        ...
        return
    print("ProjectState Isolation Release")
...
def cmd_close(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        print("This does not merge or finish the slice; use scripts/projectstate_finish_slice.py.")
    ...
    if code != 0:
        ...
        if latch_error:
            print(f"Read-only latch persistence failed: {latch_error}", file=sys.stderr)
        return 1
    finalizer = ROOT / "scripts" / "projectstate_remote_closure_finalizer.py"
    code, output, stderr = run_command(
    ...
    print("Use scripts/projectstate_finish_slice.py for agent-owned merge, main CI, and post-merge closure.")
...
def cmd_cleanup(args: argparse.Namespace) -> int:
    ...
    print("ProjectState Agent Isolation Cleanup Report (non-mutating)")
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    locks = detect_git_locks(repo)
    inventory = build_inventory(repo)
    print("ProjectState Agent Isolation Inventory")
    print("Reservations:")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ProjectState agent strong-isolation orchestrator")
    ...
    close = subparsers.add_parser(
        "close",
        help="Deprecated pre-merge push/finalizer only; use projectstate_finish_slice.py",
        description=(
            "Deprecated pre-merge push/finalizer only. This command cannot finish a slice; "
            "use scripts/projectstate_finish_slice.py for merge and post-merge closure."
        ),
    )
