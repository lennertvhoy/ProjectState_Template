#!/usr/bin/env python3
"""Machine-checkable ProjectState audit command.

This command converts ProjectState from "please be disciplined" into
...
RUNTIME_IDENTITY_SCHEMAS = {
    "projectstate.runtime_identity.v1",
    "projectstate.runtime_identity.v2",
}
EVIDENCE_MANIFEST_FILE = "manifest.json"
EVIDENCE_MANIFEST_SCHEMA = "projectstate.evidence_manifest.v1"
BROWSER_VERIFICATION_SCHEMA = "projectstate.browser_verification.v1"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Machine-checkable ProjectState audit command",
    )
...
def check_repo_role_mode(root: Path, result: AuditResult) -> None:
    ...
    if mode not in VALID_PROJECTSTATE_MODES:
        result.add("repo_role_mode", "fail", f"Missing or invalid projectstate_mode/repo_mode: {mode or 'not proven'}")
        return
    result.add("repo_role_mode", "pass", f"projectstate_mode: {mode}")

    if role == "template_repository":
        if mode != "template-maintenance":
            result.add("repo_role_mode", "fail", "template_repository must use projectstate_mode: template-maintenance")
        else:
...
def check_worktree_guard_available(repo: Path, result: AuditResult) -> None:
    guard = repo / "scripts" / "projectstate_worktree_guard.py"
    if guard.exists():
        result.add(
            "worktree_guard",
            "pass",
            "scripts/projectstate_worktree_guard.py available for pre-slice and closure worktree checks",
        )
    else:
        result.add(
            "worktree_guard",
            "warn",
            "scripts/projectstate_worktree_guard.py not found; pre-slice worktree isolation guard unavailable",
        )
...
def looks_like_schema(relpath: str) -> bool:
    if relpath.startswith("schemas/examples/") or relpath.startswith("fixtures/"):
        return False
    if Path(relpath).name in {"projectstate_validate_schema.py", "test_schema_validation.py"}:
        return False
...
def check_schema_validation(repo: Path, result: AuditResult) -> None:
    script = repo / "scripts" / "projectstate_validate_schema.py"
    if not script.exists():
        result.add("schema_validation", "fail", "Missing schema validator: scripts/projectstate_validate_schema.py")
        return
    code, stdout, stderr = run_command([sys.executable, str(script), str(repo), "--quiet"], repo)
    if code == 0:
        result.add("schema_validation", "pass", "ProjectState schema validation passed")
        return
    combined = f"{stdout}\n{stderr}".strip()
    result.add(
        "schema_validation",
        "fail",
        f"ProjectState schema validation failed ({code}): {combined[:600]}",
    )
