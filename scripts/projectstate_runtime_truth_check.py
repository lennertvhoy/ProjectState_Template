#!/usr/bin/env python3
"""Re-probe an explicit ProjectState runtime artifact against current local truth.
...
try:
    from projectstate_contracts import ContractError, confined_path, load_json_file, safe_root_path
    from projectstate_runtime_proof import (
        detect_process,
        fetch_url,
        port_from_url,
    ...
    from scripts.projectstate_contracts import ContractError, confined_path, load_json_file, safe_root_path
    from scripts.projectstate_runtime_proof import (
        detect_process,
        fetch_url,
        port_from_url,
        should_attempt_local_process_detection,
...
class RuntimeTruthCheck:
    ...
    def run(self) -> int:
        print("ProjectState Runtime Truth Check")
        try:
            payload = self._load()
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Re-probe a typed ProjectState runtime identity")
