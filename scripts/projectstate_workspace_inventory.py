#!/usr/bin/env python3
"""Inventory StateDD-managed workspaces and unexpected same-origin siblings.
...
INVENTORY_SCHEMA = "statedd.workspace_inventory.v1"
AGENT_CONTEXT_PATH = Path(".statedd/agent.context")
...
def workspace_state_root() -> Path:
    ...
    if override:
        ...
        root = (
            Path(xdg_state).expanduser() / "statedd" / "workspaces"
            if xdg_state
            else Path.home() / ".local" / "state" / "statedd" / "workspaces"
        )
    if not root.is_absolute():
        raise ValueError("StateDD workspace root must be absolute")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inventory StateDD workspace lifecycle state")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.format == "json":
        ...
        print("StateDD Workspace Inventory")
