#!/usr/bin/env python3
"""Plan and transactionally apply safe StateDD downstream asset upgrades.

The current profile catalog is desired-state authority. A validated historical
STATEDD_ASSETS.json supplies ownership and base-hash evidence. Default mode is
...
try:
    ...
    from statedd_validate_schema import load_schema as load_json_schema_contract, validate_json_schema
    from statedd_generated_controls import render_coding_agent_startup_prompt, render_downstream_workflow
    ...
    from scripts.statedd_validate_schema import load_schema as load_json_schema_contract, validate_json_schema
    from scripts.statedd_generated_controls import render_coding_agent_startup_prompt, render_downstream_workflow


TEMPLATE_ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = Path("STATEDD_ASSETS.json")
PROJECT_TRUTH_FILES = {
    Path("AGENTS.md"),
    Path("STATUS.md"),
...
def _validate_v2_payload(payload: dict[str, Any], *, label: str) -> None:
    schema = load_json_schema_contract(TEMPLATE_ROOT / "schemas" / "statedd_assets.schema.json")
    issues = validate_json_schema(payload, schema)
    if issues:
        detail = "; ".join(f"{issue.path}: {issue.message}" for issue in issues[:12])
        raise ContractError(f"{label} violates statedd.runtime_assets.v2: {detail}")
...
def load_manifest(target: Path, *, explicit_profile: str | None, catalog: dict[str, Any]) -> ManifestState:
    ...
    if not manifest_path.exists():
        if explicit_profile is None:
            raise ContractError(
                "STATEDD_ASSETS.json is missing; pass --profile explicitly to adopt a legacy/no-lock target"
            )
    ...
    if manifest_path.is_symlink() or not manifest_path.is_file():
        raise ContractError("STATEDD_ASSETS.json must be a regular file")
    payload = load_json_file(manifest_path)
    manifest_sha256 = sha256_file(manifest_path)
    if not isinstance(payload, dict):
        raise ContractError("STATEDD_ASSETS.json root must be an object")
    ...
    if schema == "statedd.runtime_assets.v1":
        ...
        retired_assets: list[dict[str, Any]] = []
        upgrade_history: list[dict[str, Any]] = []
    elif schema == "statedd.runtime_assets.v2":
        ...
        raise ContractError(f"unsupported STATEDD_ASSETS.json schema: {schema!r}")
...
def desired_generated_controls(history: ManifestState, resolved: ResolvedProfile) -> dict[Path, str]:
    ...
    needs_workflow = (
        ...
        or Path(".github/workflows/statedd-validate.yml") in history.generated_records
    )
    if needs_workflow:
        controls[Path(".github/workflows/statedd-validate.yml")] = render_downstream_workflow(
...
def _generated_record(
    ...
    return {
        ...
        **catalog["lifecycle_defaults"]["generated"],
        "schema": None,
        "generated_by": f"scripts/statedd_generated_controls.py:{relpath.name}",
...
def _manifest_record(catalog: dict[str, Any], profile: str) -> dict[str, Any]:
    ...
    return {
        ...
        **defaults,
        "schema": "schemas/statedd_assets.schema.json",
        "generated_by": "scripts/statedd_upgrade.py",
...
def execute_transaction(plan: dict[str, Any], target: Path, manifest: dict[str, Any] | None) -> None:
    ...
    if plan.get("source_manifest_present") is True:
        ...
        if (
            ...
            raise ContractError("STATEDD_ASSETS.json changed after planning")
    elif os.path.lexists(manifest_path):
        raise ContractError("STATEDD_ASSETS.json appeared after planning")
    ...
    try:
        ...
        if manifest is not None:
            ...
            print("Updated STATEDD_ASSETS.json")
...
def write_report(
    ...
    *,
    ...
    report = {
        "schema": "statedd.upgrade_report.v2",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Safe StateDD downstream upgrade planner")
