#!/usr/bin/env python3
"""Fast ProjectState health summary for humans and the CTO lane.
...
RUNTIME_IDENTITY_SCHEMA = "projectstate.runtime_identity.v1"
...
def upgrade_status(repo: Path) -> str:
    script = repo / "scripts" / "projectstate_upgrade.py"
    if not script.exists():
...
def evidence_manifest_status(repo: Path) -> str:
    ...
    if not manifest.exists():
        return "not present"
    script = repo / "scripts" / "projectstate_evidence_pack.py"
    if not script.exists():
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fast ProjectState health summary")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("ProjectState Health")
    print()
    print(f"Current HEAD: {head}")
    print(f"Branch: {branch}")
    print(f"Repo role: {role}")
    print(f"ProjectState mode: {mode}")
