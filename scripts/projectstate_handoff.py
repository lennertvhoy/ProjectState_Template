#!/usr/bin/env python3
"""Print a read-only ProjectState handoff snapshot."""
...
try:
    from projectstate_workspace_inventory import build_inventory
except ModuleNotFoundError:  # pragma: no cover
    from scripts.projectstate_workspace_inventory import build_inventory


ROOT = Path(__file__).resolve().parents[1]

AGENT_CONTEXT_SCHEMA = "projectstate.agent_context.v2"
AGENT_CONTEXT_PATH = ".projectstate/agent.context"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Print a read-only ProjectState handoff snapshot")
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run scripts/projectstate_audit.py and include its output",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("# ProjectState Handoff Snapshot")
    print()
    ...
    if args.run_audit:
        ...
        print()
        audit_script = repo / "scripts" / "projectstate_audit.py"
        if audit_script.exists():
            ...
            print("- scripts/projectstate_audit.py not found")
            verification_failed = True
