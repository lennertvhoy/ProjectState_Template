...
class EfficiencyCheck:
    ...
    def _is_alias_command(self, text: str) -> bool:
        """True if a commands/*.md file is a backward-compat alias stub.

        Alias stubs (post StateDD -> ProjectState rename) declare
        ``alias_for: <canonical>`` in frontmatter and intentionally omit the
        canonical command's full skill body, so they must be excluded from
        structural, size, and discovery checks that the canonical command
        already satisfies.
        """
        return bool(self._parse_frontmatter(text).get("alias_for"))
    ...
    def check_instruction_sizes(self) -> None:
        ...
        for path in self.root.glob("commands/*.md"):
            ...
            if self._is_alias_command(text):
                continue
            lines = self._line_count(path)
    ...
    def _reference_corpus(self, *extra: Path) -> str:
        ...
        for cmd in self.root.glob("commands/*.md"):
            ...
            if self._is_alias_command(cmd_text):
                continue  # alias stubs are not part of the discoverable corpus
    ...
    def check_unreferenced_skills_commands(self) -> None:
        ...
        for cmd_path in self.root.glob("commands/*.md"):
            ...
            # Backward-compat alias files (post StateDD -> ProjectState rename)
            # are intentionally not advertised in AGENTS.md/README.md; skip them.
            if front.get("alias_for"):
                continue
    ...
    def check_single_file_edit_audit(self) -> None:
        ...
        for cmd_path in self.root.glob("commands/*.md"):
            ...
            if self._is_alias_command(text):
                continue
            lower = text.lower()
            if "single file" in lower or "minor edit" in lower or "one file" in lower:
                if any(phrase in lower for phrase in self.FULL_GATE_PHRASES):
                    self._log(
                        Severity.ERROR,
                        "audit_escalation",
