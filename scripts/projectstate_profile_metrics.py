...
try:
    from projectstate_contracts import ContractError, load_profile_catalog, resolve_profile
    ...
    from scripts.projectstate_contracts import ContractError, load_profile_catalog, resolve_profile
    from scripts.projectstate_validate_schema import ProjectStateYamlError, parse_yaml_text


ROOT = Path(__file__).resolve().parents[1]
SCHEMA = "projectstate.profile_metrics.v1"
...
def normalized_file_blobs(
    ...
    manifest_blob = blobs.get("PROJECTSTATE_ASSETS.json")
    ...
    for record in manifest.get("managed_assets", []):
        ...
        if not isinstance(rel, str) or rel == "PROJECTSTATE_ASSETS.json" or rel not in blobs:
            continue
    ...
    blobs["PROJECTSTATE_ASSETS.json"] = (
...
def source_tree_digest(root: Path, catalog: dict[str, Any]) -> tuple[str, list[str]]:
    inputs = {
        ...
        "scripts/projectstate_contracts.py",
        "scripts/projectstate_profile_metrics.py",
...
def measure_profile(
    ...
    with tempfile.TemporaryDirectory(prefix="projectstate-profile-metrics-") as raw_tmp:
        ...
        lock = json.loads((target / "PROJECTSTATE_ASSETS.json").read_text(encoding="utf-8"))
        gate_level = lock["required_gate_level"]
        gate_command = [
            sys.executable,
            str(target / "scripts" / "projectstate_quality_gate.py"),
            "--root",
            str(target),
            "--gate-level",
            str(gate_level),
        ...
        return {
            ...
            "quality_gate": {
                ...
                "command": "python3 scripts/projectstate_quality_gate.py --gate-level "
                + str(gate_level)
                + " --conformance",
            },
            "contexts": contexts,
...
def build_metrics(root: Path, *, template_commit: str, epoch: int) -> dict[str, Any]:
    ...
    return {
        ...
        "generation_command": " ".join(
            [
                "python3 scripts/projectstate_profile_metrics.py",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate reproducible ProjectState profile/context metrics")
