#!/usr/bin/env python3
"""Shared strict-contract and path-confinement primitives for ProjectState.

This is a library, not a CLI. ProjectState-controlled JSON must reject duplicate
...
class ContractError(ValueError):
    """Raised when a ProjectState-controlled contract is malformed or unsafe."""
...
VALIDATION_REQUIREMENTS: dict[str, dict[str, Any]] = {
    ...
    "git_safety_contract": {
        "minimum_gate_level": 1,
        "required_paths": (
            "scripts/projectstate_git_safety_check.py",
            "scripts/projectstate_git_safety_session.py",
    ...
    "bootstrap_contract": {
        "minimum_gate_level": 1,
        "required_paths": (
            "scripts/projectstate_bootstrap_apply.py",
    ...
    "remote_closure_contract": {
        "required_paths": ("scripts/projectstate_remote_closure_finalizer.py",),
    },
    ...
    "worktree_contract": {
        "required_paths": (
            ...
            "scripts/projectstate_agent_worktree.py",
            "scripts/projectstate_workspace_inventory.py",
        ),
    },
    ...
    "post_merge_contract": {
        "required_paths": ("scripts/projectstate_post_merge_verify.py",),
    },
    ...
    "okf_v0_1_conformance": {
        "minimum_gate_level": 1,
        "required_paths": (
            "scripts/projectstate_okf_validate.py",
            "schemas/projectstate_okf_extension.schema.json",
            "knowledge/index.md",
        ),
    },
}
...
def load_profile_catalog(root: Path) -> dict[str, Any]:
    ...
    if payload.get("schema") != "projectstate.profile_catalog.v1":
...
ASSETS_MANIFEST_CANONICAL = "PROJECTSTATE_ASSETS.json"
ASSETS_MANIFEST_LEGACY = "STATEDD_ASSETS.json"


def resolve_assets_manifest(root: Path) -> Path | None:
    """Return the canonical assets manifest path if present, else the legacy alias.

    Kept for one migration cycle so downstream projects that still ship
    ``STATEDD_ASSETS.json`` continue to validate and upgrade cleanly. Returns
    ``None`` when neither file exists.
    """
    canonical = confined_path(root, ASSETS_MANIFEST_CANONICAL)
    if canonical.exists():
        return canonical
    legacy = confined_path(root, ASSETS_MANIFEST_LEGACY)
    if legacy.exists():
        return legacy
    return None
