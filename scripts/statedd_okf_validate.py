#!/usr/bin/env python3
"""Validate an optional OKF v0.1 knowledge bundle and StateDD governance.

The validator intentionally uses only the Python standard library plus the
already-shipped StateDD YAML subset parser. OKF's base contract is permissive:
unknown types, unknown frontmatter fields, broken links, and missing indexes are
warnings. The namespaced ``statedd`` extension adds ownership, provenance, and
    ...
    if value.startswith("{") and value.endswith("}"):
        ...
        for item in _split_flow_items(value):
            ...
            if not key or key in result:
                raise StateDDYamlError(f"duplicate or empty flow mapping key {key!r}")
...
def parse_frontmatter(text: str, path: Path) -> tuple[dict[str, Any], str]:
    if not text.startswith("---\n") and text != "---":
        raise StateDDYamlError(f"{path}: missing YAML frontmatter opening delimiter")
    ...
    if closing_index is None:
        raise StateDDYamlError(f"{path}: missing YAML frontmatter closing delimiter")
    raw = "".join(lines[1:closing_index])
    # The shared StateDD YAML subset intentionally treats ``- key: value`` as
    ...
    # parser used by the rest of StateDD.
    raw = re.sub(r"(?m)^(\s*)-\s+(https?://[^\s#]+)\s*$", r'\1- "\2"', raw)
    data = _expand_flow_values(parse_yaml_text(raw))
    if not isinstance(data, dict):
        raise StateDDYamlError(f"{path}: frontmatter must be a mapping")
...
def validate_source(source_root: Path, source: dict[str, Any], path: Path, report: Report) -> None:
    if not isinstance(source, dict):
        report.error(path, "each statedd.sources entry must be a mapping")
        return
...
def check_governance(
    ...
    extension = frontmatter.get("statedd")
    if extension is None:
        return
    if not isinstance(extension, dict):
        report.error(path, "statedd extension must be a mapping")
        return
    authority = extension.get("authority")
    if authority not in {"canonical", "derived", "reference"}:
        report.error(path, "statedd.authority must be canonical, derived, or reference")
        return
    if authority == "canonical":
        if not isinstance(extension.get("owner"), str) or not extension["owner"].strip():
            report.error(path, "canonical concepts require a non-empty statedd.owner")
        parse_timestamp(extension.get("reviewed_at"), "statedd.reviewed_at", path, report)
    elif authority == "derived":
        sources = extension.get("sources")
        if not isinstance(sources, list) or not sources:
            report.error(path, "derived concepts require a non-empty statedd.sources list")
        else:
        ...
        if not isinstance(citations, list) or not citations or not all(
            ...
            report.error(path, "reference concepts require a non-empty statedd.citations list")
        parse_timestamp(extension.get("last_checked_at"), "statedd.last_checked_at", path, report)
...
def validate_bundle(bundle: Path, source_root: Path) -> Report:
    ...
    for path in markdown_files:
        ...
        try:
            frontmatter, body = parse_frontmatter(text, path)
        except (StateDDYamlError, UnicodeDecodeError) as exc:
            report.error(relative, str(exc))
            continue
...
def output_report(report: Report, *, as_json: bool = False) -> None:
    ...
    if not report.errors:
        print("Base OKF conformance and StateDD governance checks passed.")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate an OKF v0.1 bundle and StateDD governance metadata")
    parser.add_argument("bundle", help="Bundle directory, normally knowledge/")
    parser.add_argument("--source-root", help="Repository root used to resolve statedd.sources paths")
