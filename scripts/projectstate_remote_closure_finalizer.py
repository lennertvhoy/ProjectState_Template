#!/usr/bin/env python3
"""ProjectState Remote Closure Finalizer.
...
try:
    from projectstate_contracts import ContractError, UnsafePathError, confined_path, load_json_file, normalize_relative_path
    from projectstate_evidence_pack import command_check as check_evidence_pack, scan_text_file
    from projectstate_validate_schema import ProjectStateYamlError, parse_yaml_text, validate_file
except ModuleNotFoundError:  # pragma: no cover - pytest package import path
    from scripts.projectstate_contracts import ContractError, UnsafePathError, confined_path, load_json_file, normalize_relative_path
    from scripts.projectstate_evidence_pack import command_check as check_evidence_pack, scan_text_file
    from scripts.projectstate_validate_schema import ProjectStateYamlError, parse_yaml_text, validate_file
...
AGENT_CONTEXT_SCHEMA = "projectstate.agent_context.v2"
AGENT_CONTEXT_FILE = Path(".projectstate/agent.context")
    ...
    def _query_urllib(self, query: str, variables: dict[str, Any]) -> dict[str, Any]:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "projectstate-remote-closure-finalizer",
            },
            method="POST",
        )
...
class RemoteClosureFinalizer:
    ...
    def run(self) -> int:
        print("=" * 60)
        print("ProjectState Remote Closure Finalizer")
        print("=" * 60)
    ...
    def _resolve_workflow_path(self) -> None:
        ...
        manifest_path = self.root / "PROJECTSTATE_ASSETS.json"
        ...
        if manifest_path.is_file() and not manifest_path.is_symlink():
            ...
            if not isinstance(manifest, dict) or manifest.get("schema") != "projectstate.runtime_assets.v2":
                raise RuntimeError("Authoritative workflow requires a v2 PROJECTSTATE_ASSETS.json lock")
            ...
            declared_records = [
                ...
                and record.get("path") == ".github/workflows/projectstate-validate.yml"
            ]
            if len(declared_records) != 1:
                raise RuntimeError(
                    "Asset lock must declare exactly one generated ProjectState validation workflow"
                )
        ...
        except (UnicodeDecodeError, ProjectStateYamlError) as exc:
        ...
        if len(matching) != 1:
            raise RuntimeError(
                "Authoritative workflow must invoke exactly one unconditional ProjectState quality gate "
    ...
    def _write_output(self) -> None:
        artifact = {
            "schema": "projectstate.remote_closure_handoff.v1",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="ProjectState Remote Closure Finalizer: verify pushed PR/CI state before calling a slice done"
    )
    ...
    parser.add_argument(
        "--agent-context",
        default=None,
        help="Path to agent.context JSON (auto-detects .projectstate/agent.context if omitted)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        from projectstate_contracts import safe_root_path
    except ModuleNotFoundError:  # pragma: no cover
        from scripts.projectstate_contracts import safe_root_path
    try:
